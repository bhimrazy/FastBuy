<div>
    ...
    <form action="{{ route('products.search') }}" class="hm-searchbox" style="z-index: 10" method="post">
        @csrf
        <select name="category_id" wire:model="subcategory" class="nice-select select-search-category">
        ...
        <input name="query" type="text" wire:model.debounce.500ms="search"
            placeholder="{{ $search ?? 'Enter your search key ...' }}" required>
        ...
        <div class="position-relative category-menu col-6 ml-150" style="z-index: 8;background-color: #232f3e;">
            <div class="category-menu-list">
                <ul class="mt-20" style="height: 0">
                    @foreach ($searchProducts->take(5) as $product)
                        <li class="border-bottom p-2 py-3 m-auto ">
                            <a href="{{ route('product.productShow', ['product' => $product['slug']]) }}">
                                <img class="left" src="{{ asset($product->media->first()->url) }}" loading="lazy"
                                    width="40px" height="auto" alt="{{ $product->name }}">
