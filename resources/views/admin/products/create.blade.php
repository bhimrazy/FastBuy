...
<div class="content-wrapper">
    ...
    <section class="content">
        <form role="form" enctype="multipart/form-data" action="{{route('admin.products.store')}}" method="POST">
            @csrf
            <div class="row">
                <div class="col-md-6">
                    <div class="card card-primary">
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="attributes" name="attributes" class="attributes form-control" rows="6" required>{{old('attributes')}}</textarea>
                                @error('attributes')
                                <small class="text-danger">{{$message}}</small>
                                @enderror
                            </div>
                                ...
                                <select id="inputCategory" name="subcategory_id" class="form-control custom-select" required>
                                    ...
                                    @foreach($categories as $category)
                                        ...
                                        @foreach($category->subcategories as $subcategory)
                                            <option class="badge text-success" value="{{$subcategory->id}}" {{ $subcategory->id==old('subcategory_id') ? 'selected' : '' }}>{{$subcategory->title}}</option>
                                        @endforeach
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                @foreach($tags as $tag)
                                    <div class="icheck-primary custom-control custom-checkbox">
                                        <input type="checkbox" name="tags[]" id="checkboxPrimary1{{$tag->id}}" value="{{$tag->id}}" {{ in_array($tag->id, old('tags', [])) ? 'checked' : '' }}>
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <div class="custom-file">
                                    ...
                                    <input class="form-control p-1" id="fileToUpload" type="file" name="image[]" onchange="load(event,'product-img')" accept=".png, .jpg, .jpeg" multiple required>
                                </div>
                    ...
                    <div class="card card-secondary">
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <div class="icheck-primary custom-control custom-checkbox">
                                    <input type="checkbox" name="featured" id="checkboxPrimary1featured" {{old('featured')?'checked':''}}>
                                ...
                                @enderror
                                <div class="icheck-primary custom-control custom-checkbox">
                                    <input type="checkbox" name="status" id="checkboxPrimary1status" {{old('status')?'checked':''}}>
                            ...
                            <div class="form-group">
                                ...
                                <div class="icheck-primary custom-control custom-checkbox">
                                    <input type="checkbox" name="stock" id="checkboxPrimary1stock" {{old('stock')?'checked':''}}>
                            ...
                            <div class="form-group">
                                ...
                                <select id="inputCategory" name="brand_id" class="form-control custom-select" required>
                                    ...
                                    @foreach($brands as $brand)
                                        <option class="text-bold text-gray-dark" value="{{$brand->id}}" {{$brand->id==old('brand_id')?'selected':''}}>{{$brand->title}}</option>
                                    @endforeach
                                </select>
                                @error('brand_id')
                                <small class="text-danger">{{$message}}</small>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="product_type" id="product_type" class="form-control" value="{{old('product_type')}}" required>
                                @error('product_type')
                                <small class="text-danger">{{$message}}</small>
                                @enderror
                            </div>
